...
function placedetails(api::GoogleAPI, area::Meshes.MultiPolygon, api_limit::Integer = 4000)
...
function _createsearchpoints(
    area::Meshes.MultiPolygon,
    api_limit::Integer,
)::Vector{Meshes.Point2}
...
function _nearbysearch(; api::GoogleAPI, search_point::Meshes.Point)::Vector{Any}
    ...
    search_circle::Dict{String,Any} = Dict(
        "center" =>
            Dict{String,Float64}("latitude" => coords[2], "longitude" => coords[1]),
        "radius" => 500,
    )

    body::Dict{String,Any} = Dict(
        "rankPreference" => "DISTANCE",
        "locationRestriction" => Dict{String,Dict}("circle" => search_circle),
    )
